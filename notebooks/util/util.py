...
def plot_histogram(data, bins=10, vmin=None, vmax=None, figsize=figsize, marked_values=None):
    # Build a new figure
    plt.close('all')
    plt.figure(figsize=figsize)
    # Plot a histogram
    ...
    if vmax is not None:
        lims = (lims[0], vmax)
    plt.xlim(lims)
    # Plot vertical lines at the marked values, if any
    if marked_values is not None:
        ymin, ymax = plt.ylim()
        plt.vlines(marked_values, ymin, ymax, color='tab:orange')
        plt.ylim(ymin, ymax)
    plt.grid(':')
    plt.tight_layout()
